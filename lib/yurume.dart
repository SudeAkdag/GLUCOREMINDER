...
class _YurumeSayfasiState extends State<YurumeSayfasi>
  ...
  Future<void> _kontrolVeSifirla() async {
    ...
    final docRef =
        FirebaseFirestore.instance.collection('yurume_verileri').doc(tarihKey);
    ...
    if (!doc.exists) {
      ...
      final timerRunning =
          (data['zamanlayici_durum'] ?? false) == true; // ← BURAYA EKLE
...
// 📊 Son 7 gün için kalori verisini al ve gruplandır
  Future<void> _loadLast7Calories() async {
    ...
    for (var doc in docs) {
      ...
      final dayKey =
          "${zaman.year}-${zaman.month.toString().padLeft(2, '0')}-${zaman.day.toString().padLeft(2, '0')}";

      final kalori = (data['kalori'] as num).toDouble();
      final zamanlayiciDurum = data.containsKey('zamanlayici_durum')
          ? data['zamanlayici_durum'] as bool
          : false;

      if (zamanlayiciDurum) {
        // Sayaç devam ediyorsa aynı güne ekle
        groupedCalories.update(dayKey, (value) => value + kalori,
            ifAbsent: () => kalori);
      } else {
  ...
  Future<void> _kaydetYurumeVerisi({required bool timerRunning}) async {
    final bugun = DateTime.now();
    final tarihKey =
        "${bugun.year}-${bugun.month.toString().padLeft(2, '0')}-${bugun.day.toString().padLeft(2, '0')}";
    final docRef =
  ...
  void startTimer() {
    ...
    _kaydetYurumeVerisi(timerRunning: true); // ZAMANLAYICI DURUMU TRUE
  }
  ...
  void stopTimer() {
    ...
    _kaydetYurumeVerisi(timerRunning: false); // ZAMANLAYICI DURUMU FALSE
  }
                  ...
                  child: Padding(
                    ...
                    child: SizedBox(
                      ...
                      child: last7Calories.isEmpty || last7DaysNames.isEmpty
                          ? Center(
                              child:
                                  CircularProgressIndicator()) // Yükleniyor ekranı
                          : LineChart(
                              LineChartData(
                                minX: 0,
